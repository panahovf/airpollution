...
df_concentration_cp = df_concentration_cp[df_concentration_cp['GU_A3'].isin(countries)]
df_concentration_nz = df_concentration_nz[df_concentration_nz['GU_A3'].isin(countries)]
df_concentration_max = df_concentration_max[df_concentration_max['GU_A3'].isin(countries)]
...
# --------------
# STEP 2: Get global decarbonization pathway
...
df_max_power = df_max_power.groupby('fuel_type')[year_columns].sum().reset_index()
...
# --------------
# STEP 3: Get CP vs NZ difference: (1-NZ)/Current levels
...
#for country in countries:
    ...
    for year in year_columns:
        ...
        df_concentration_nz_total[year] = df_concentration_nz_total['Current_level'] - (df_concentration_nz_total['ENEcoal'] * coal_factor + df_concentration_nz_total['ENEother'] * oilgas_factor)
...
#for country in countries:
    ...
    for year in year_columns:
        ...
        # Calculate reductions
        df_concentration_cp_total[year] = df_concentration_cp_total['Current_level'] - (df_concentration_cp_total['ENEcoal'] * coal_factor + df_concentration_cp_total['ENEother'] * oilgas_factor)
...
#for country in countries:
    ...
    for year in year_columns:
        ...
        # Calculate reductions
        df_concentration_max_total[year] = df_concentration_max_total['Current_level'] - (df_concentration_max_total['ENEcoal'] * coal_factor + df_concentration_max_total['ENEother'] * oilgas_factor)
...
aaa = df_concentration_nz_total.copy()
bbb = df_concentration_cp_total.copy()
...
df_concentration_cp_total.to_excel('2 - output/script 4/s4.10_- 1.1 - annual concentration - grid level - current policy.xlsx', index = False)
df_concentration_nz_total.to_excel('2 - output/script 4/s4.10 - 1.2 - annual concentration - grid level - net zero 1.5C.xlsx', index = False)
df_concentration_max_total.to_excel('2 - output/script 4/s4.10 - 1.3 - annual concentration - grid level - max shut down.xlsx', index = False)

# --------------
# annual concentration levels - country
df_concentration_cp_country.to_excel('2 - output/script 4/s4.10 - 2.1 - annual concentration - country level - current policy.xlsx', index = False)
df_concentration_nz_country.to_excel('2 - output/script 4/s4.10 - 2.2 - annual concentration - country level - net zero 1.5C.xlsx', index = False)
df_concentration_max_country.to_excel('2 - output/script 4/s4.10 - 2.3 - annual concentration - country level - max shut down.xlsx', index = False)
